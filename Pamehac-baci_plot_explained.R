...
ggplot(df, aes(x = reorder(time, desc(time)), y = rv)) +
  ...
  ylim(0, 7) +
  theme_minimal() +
  annotate("text", x = 2.3, y = 2, label = "Above") +
  annotate("text", x = 2.3, y = 5, label = "Below") +
  annotate("text", x = 2, y = 1.5, label = "Intercept") +
  annotate("text", x = 2, y = 4.5, label = "Intercept + type") +
  annotate("text", x = 1, y = 6, label = "Intercept + type \n + time + trt:time") +
