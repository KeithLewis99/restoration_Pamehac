...
Summary of issues: specifically the density for total salmonids is less than the sum of the total and the density for BT in 1996 above the diversion appears to be an unscaled count which calls into question the claim that juvenile salmonids demonstrated a 18.3 fold increase by 1996.
...
So, I think its best just to keep 5A and 8A - discussed with Clarke and Kristin at meeting in early Sept.
...
But note Barket et al. [-@barker2018reliability] severly criticize these approaches.  I’m not sure if this extends to the other methods but imagine that it must since p is always going to be a latent variable, i.e., these are CMR models but without a means to directly estimate p which is always going to be a problem.  Quote: "Our message is that fundamentally count data under imperfect detection can only be reliably used as indices. Having controlled for detectability using covariates, counts through ratios of their expectations provide reliable information about relative abundance, but not absolute abundance N or p.” They do advocate for a CMR approach if absolute abundance is needed.  

Bayesian and other approaches are in Rcode_archive.  
...
### Paul Regular
2024-10-17 - Paul gave me almost 2 hours of his time.  I explained what I was up to and we went through the various iterations [mostly, I think, on the Richter approach].  Paul wondered if the model was over paramaterized but was positive about the approach in general. 
...
### Dave Fifield
We discussed BLUPs which DAve confirmed are what is generated by ranef().  He wasn't familiar with the "sires" origins of LMM.  We also talked about using the ziformula of the hurdle model.  He agreed that we can use taht to say that there are more zeros in one period compared to another.  
In the course of the discussion, we found that for Pamehac, the BLUP for 1990 is always ~ 0.  I checked if changing the ziformula model made any difference and it didn't.  I checked other study areas (Granite Canal) and it doesn't have the same problem.  Not much online about this.  ChatGPT and Google AI suggest a reference level thing like for fixed effects or that it really is close to zero.  I pushed back and ChatGPT suggested that its really just because its close to zero. Interpret this, not as a normal NHST, but merely as evidence that a given year differs.  Its a suggestion, not a formal test.  Could have Year as a conditional effect but Year as a factor gets dropped, I suspect due to lack of data and Year as a continuous variable really only makes sense if terminal ends are max/min respectively.  

Also, only interpret ziformula when a) not an intercept (zeros apply to all variables evenly), b) the term is significant, and c) only for density.  I can't see that interpretting for biomass is very meaningful but think on this.  

Also, Fifield uses function map a lot.  Seems like a good way to produce publication quality images when facet_grid won't quite do it and to reduce code.

### Guillaume Dauphin
Met on 2026-05-21 with Guillaume who was a RES in Gulf region and did a lot of work on salmon and electrofishing.  He didn't disagree with my approach (pick your poison) but had great questions. He kindly offered to look at some of these data and see if he could do anything with it. But the following sums up our discussion:   
- He was not horrified by the SPC graphs but acknowledged tha there is a problem.  
- He emphasized that effort spent electrofishing is important.  
- Suggested grouping the AS and BT by age-1+ and YOY under the assumption that q would be more or less equal for each age class.  
- suggested a hierarchical approach to estimating q, i.e., estimating by site within year, averaging, and then adding covariates.  
- did the electrofisher and upstream v downstream approaches change over time?  This can affect estimates.
- for T v No, he suggested doing a CI and checking the sites or removing data points and rechecking.

### Guillaume Dauphin II
Guillaume did up a great Shiny app and implemented all the code for a Bayesian-MMI approach to depletion sampling and density estimation. We met again (2026-04-27).  
- The good news is that the values for M7 are generally just a bit above T (total catch) and the relation between these values are just a bit more than 1:1.
- He did the Shiny app in Claude.  
- Loops could have been done differently in order to better match MARK and stock assessment notation but he did it very quickly.  But site-year and species are both combinations of variables - but its better to not use too many loops.  
- the Bayesian-MMI approach is great.  Even though M7 is only the best of the hiearchical models, the partial models have really wide credible intervals.  
 - feed results into glmmTMB but Guillaume is working on other 
...
nrow(df_out) # FSA output
```

### low n
Difference between df_a and df_out is that Station == "5" and Station == "9B" have been removed from df_a. Use df_a
```{r}
nrow(df_a |> filter(abun < 30)) # 143 of 168
nrow(df_a |> filter(abun < 20)) # 132 of 168
nrow(df_a |> filter(abun < 10)) # 105 of 168
nrow(df_a |> filter(abun < 5)) # 81 of 168

nrow(df_out |> filter(T < 30)) # 153 of 180
nrow(df_out |> filter(T < 20)) # 141 of 180
nrow(df_out |> filter(T < 10)) # 113 of 180
nrow(df_out |> filter(T < 5)) # 89 of 180
```
...
Spent some time looking at replacement.  Ran into the problem of figuring out what Scruton did because a lot of the numbers in the paper do not add up to what I have.  This required going back to the notebooks and worksheets and spreadsheets.  The notebooks (and worksheets) do not list species-age classes.  The just give the species and length.  But, in the spreadsheets, it appears that the determination of age-class had been made.  Also, areas match between all three (at least the ones I checked) so we are assuming that the data are correct, that Scruton's calculations are wrong, and that the paper is being billed as a re-analysis of his work.  

Also, learned that the sum of biomass of all species divided by the area * 1/4 is that same as the sum of the biomass of each species divided by the area.  Keep getting this confused.  One thing that i'm sure that SCruton did not do is add biomass/area up by Station for each species.  
