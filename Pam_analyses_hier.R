...
library(dplyr)
df_a <- read.csv("data_derived/N_species_comparison.csv") |>
  filter(!(station == "5B" | station == "9")) |>
  filter(model == "M7_hier_p_global")
str(df_a)


df_a3 <- read.csv("data_derived/df_a3.csv") |>
  select(Year, Station, Species, abun, abun.stand, bio.stand, time, type, area, west, north)

df_a <- left_join(df_a, df_a3, by = c("station" = "Station", 
                                     "year" = "Year", 
                                     "species" = "Species"
                                     ))
...
plot(density(df_aBT$density, na.rm = T))
summary(df_aBT$abun.stand)
with(df_aBT, table(abun, Year))
with(df_aBT, table(abun, Station, Year))
ggplot(df_aBT, aes(x = station, y = density)) + geom_boxplot() + facet_wrap(~year)
...
bt.glmm1 <- glmmTMB(
  density ~ type*time + (1 | year), #+ (1 | Station),
...
df_aBT |> group_by(time) |> count(abun == 0) 
df_aBT |> group_by(year) |> summarise(meanYear = mean(density))
...
btyoy.glmm1 <- glmmTMB(
  # density ~ type*time + as.factor(year) + (1 | year),
  density ~ type*time + (1 | year),
  dispformula = ~ int,
  family=ziGamma(link="log"), 
  ziformula = ~ 1,
...
df_aBTYOY |> filter(time == "before" & type == "below")
df_aBTYOY |> filter(time == "before" & type == "above")

df_aBTYOY |> filter(time == "after" & type == "below")
df_aBTYOY |> filter(time == "after" & type == "above")

df_aBTYOY |> group_by(time) |> count(abun == 0) 
df_aBTYOY |> group_by(Year) |> summarise(meanYear = mean(abun.stand))
...
VarCorr(best_model)
df_aBT |> group_by(Year) |> summarise(meanYear = mean(abun.stand))
...
df_aASYOY |> group_by(Year) |> summarise(meanYear = mean(abun.stand))
...
#### zi ----
summary(best_model)$coefficients$zi
plogis(summary(best_model)$coefficients$zi)
plogis(-0.8649972 + 2.6567588) # this matches 6/7 stations before had zero
