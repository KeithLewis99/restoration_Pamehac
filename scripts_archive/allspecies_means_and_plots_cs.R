...
###Check to make sure only salmon
pam_density_allAS

jpeg("Pam_density_plot.jpg", width=6.5, height=4, units='in', res=600)

Pam_salmondense_plot<-ggplot(pam_density_allAS, aes(Year, mean, colour=type)) + 
  ...
  geom_vline(xintercept = 3.5, linetype="dashed", size=0.5, angle=90) +
  geom_vline(xintercept = 4.5, linetype="dashed", size=0.5, angle=90) +
  geom_errorbar(aes(ymax=mean-se, ymin=mean+se), size=0.5, width=0.25, position=position_dodge(1)) +
  theme(panel.grid.minor=element_blank(), panel.grid.major=element_blank())
...
AS_dense_plot<-ggplot(pam_ASbiomass_se, aes(Year, mean, colour=type)) + 
